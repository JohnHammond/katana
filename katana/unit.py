...
from typing import Any, List, Type, Tuple, IO
import subprocess
import importlib
import logging
import pkgutil
...
class Unit(object):
	...
	def __init__(self, manager: katana.manager.Manager,
		...
		if target.parent is not None and self.PROTECTED_RECURSE and \
				target.parent.PROTECTED_RECURSE:
	...
	def evaluate(self, case: Any):
		...
		raise RuntimeError('{0}: malformed unit: no evaluate'.format(
			self))
	...
	def generate_artifact(self, name: str = None, mode: str = 'w',
			create: bool = True, asdir: bool = False) -> Tuple[str,IO]:
...
class Finder(object):
	...
	def __init__(self, manager: katana.manager.Manager, use_default: bool = True):
		...
		if use_default:
			for unit in self.find(os.path.join(katana.__path__[0], 'units'),
							'katana.units.'):
				self.register(unit)
	...
	def find(self, directory: str, prefix: str) -> Generator[Type[Unit], None, None]:
		...
		for importer, name, ispkg in pkgutil.walk_packages([directory], prefix):
			...
			# Ensure this unit is not on the exclusion list
			try:
				# Check all the excluded modules against this one
				for exclude in self.manager['manager']['exclude']:
					if name == exclude or \
							name.startswith(exclude.rstrip('.')+'.'):
						raise NotApplicable
			except NotApplicable:
				# Ignore excluded units
				continue
