...
class Unit(WebUnit):
    ...
    def evaluate(self, katana, case):
        ...
        try:
            fetch_git(self, self.target.url_root, git_directory, katana.config['git_jobs'], katana.config['git_retry'],
                      katana.config['git_timeout'], katana)
        ...
        grep = subprocess.run("git grep -P {}".format(katana.config['flag_format']).split(), cwd=git_directory,
        ...
        for line in grep.stdout.decode('utf-8').split('\n'):
            ...
            if katana.config['flag_format']:
                commit_data = subprocess.run(f"git checkout {commit}".split(), cwd=git_directory,
